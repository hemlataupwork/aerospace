...
$url = new moodle_url('/admin/roles/assignpoc.php', array('contextid' => $contextid));
...
if($_POST){
    ...
    if($_POST['addselect']){
        global $DB;
    $contextid1 = context_coursecat::instance($_POST['schoolid']);
    foreach($_POST['addselect'] as $user){  
        role_assign($roleid, $user, $contextid1->id, '', 0); 
        $DB->execute("UPDATE {poc} SET roleid=$roleid,contextid=$contextid1->id WHERE userid=$user");
    }
    
} elseif($_POST['removeselect']){
    
    $contextid1 = context_coursecat::instance($_POST['schoolid']);
    foreach($_POST['removeselect'] as $user){
        role_unassign($roleid, $user, $contextid1->id, '', 0);         
        // echo 'hiooo';
        // die;
        $DB->execute("UPDATE {poc} SET roleid=0,contextid=0 WHERE userid=$user");
        }
        }
...
if ($roleid) {
    ...
    $options = array('context' => $context, 'roleid' => $roleid,'coursecatid' => $_POST['schoolid']);
    ...
    $currentuserselector = new core_role_existing_role_holders('removeselect', $options);
        ...
        // print_r($currentuserselector->display());
        // echo 'hi';
        // die;
...
if ($backurl) {
    $backurl1 = new moodle_url('/local/regionalpoc/regionalpoc_custom.php');
    $backbutton = new single_button($backurl1, get_string('back'), 'get');
...
if ($roleid) {
    ...
    // Print the form.
    $assignurl = new moodle_url($url, array('roleid'=>$roleid));
...
<form id="assignform" method="post" action="<?php echo $assignurl ?>"><div>
  ...
  <select name="schoolid" class="form-control custom-select mr-1" id="selectschool" required >
    <?php
        echo "<option>Select School</option>";
    foreach ($schools as $school => $label) {
        echo "<option value=\"$school\">$label</option>";
    }
    ?>
</select>
...
<script>
   document.addEventListener('DOMContentLoaded', (event) => {
            var selectSchoolElement = document.getElementById('selectschool');
            if (selectSchoolElement) {
                selectSchoolElement.addEventListener('change', function() {
                    console.log(selectSchoolElement.value);
                });
            } else {
                console.error('Element with ID "selectschool" not found.');
            }
        });
</script>
...
if (!empty($errors)) {
    ...
    $urltopoc="$CFG->wwwroot/admin/roles/assignpoc.php?contextid=$contextid";
    // var_dump($urltopoc);
    // die;
    $PAGE->set_url($urltopoc);
