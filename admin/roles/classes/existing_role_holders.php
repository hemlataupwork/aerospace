...
class core_role_existing_role_holders extends core_role_assign_user_selector_base {
    ...
    public function find_users($search) {
        ...
        // SELECT ara.userid,ara.roleid,ac.instanceid FROM aero_role_assignments ara join aero_context ac on ac.id = ara.contextid  where  ac.contextlevel = 40 



        $fields = "SELECT ra.id AS raid, u.id, " . $this->userfieldsselects . ", ra.contextid, ra.component,ctx.instanceid ";
        ...
        // $fields = "SELECT ara.userid, ara.roleid," . $this->userfieldsselects . ", ctx.instanceid";
        // $countfields = "SELECT COUNT(1) ";
        //    $sql = "FROM {role_assignments} ra
        //           JOIN {user} u ON u.id = ra.userid
        //           JOIN {context} ctx ON ra.contextid = ctx.id
